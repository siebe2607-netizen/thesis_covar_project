...
print("========================================================")
print("PHASE 2 & 3: THE FINAL FORWARD-COVAR EXECUTION")
print("========================================================\n")

window = 250
delta_covar_dict = {t: [] for t in tickers}
calc_indices = []

print("Step 2: Executing 250-Day Rolling Quantile Regressions...")

for i in range(window, len(full_df)):
    window_data = full_df.iloc[i-window:i]
    calc_indices.append(full_df.index[i])

    X_macro = sm.add_constant(window_data[macro_vars])

    # 🚨 THE FIX: Manually create the prediction row with a constant
    pred_X = full_df.iloc[i:i+1][macro_vars].copy()
    pred_X.insert(0, 'const', 1.0)

    for t in tickers:
        try:
            y_asset = window_data[f'{t}_Ret']
            y_sys = window_data[f'sys_ret_excl_{t}']

            # --- Stage 1: Asset VaR ---
            model_var_5 = QuantReg(y_asset, X_macro).fit(q=0.05, p_tol=1e-4, max_iter=2000)
            model_var_50 = QuantReg(y_asset, X_macro).fit(q=0.50, p_tol=1e-4, max_iter=2000)

            # Predict using the manually constructed pred_X
            var_5 = model_var_5.predict(pred_X).iloc[0]
            var_50 = model_var_50.predict(pred_X).iloc[0]

            # --- Stage 2: System CoVaR ---
            X_sys = sm.add_constant(pd.concat([window_data[f'{t}_Ret'], window_data[macro_vars]], axis=1))
            model_covar = QuantReg(y_sys, X_sys).fit(q=0.05, p_tol=1e-4, max_iter=2000)

            beta = model_covar.params[f'{t}_Ret']

            # Calculate Delta CoVaR
            delta_covar = beta * (var_5 - var_50)
            delta_covar_dict[t].append(delta_covar)

        except Exception as e:
            # If a day genuinely fails, append NaN
            delta_covar_dict[t].append(np.nan)

df_dcovar = pd.DataFrame(delta_covar_dict, index=calc_indices)
print("-> Successfully extracted time-varying Delta CoVaR series!\n")

# ==========================================
# STEP 3: FORWARD-COVAR EVALUATION
# ==========================================
print("Step 3: Evaluating Out-of-Sample Pinball Loss...\n")

def pinball_loss(y_true, y_pred, q=0.05):
    error = y_true - y_pred
    return np.mean(np.maximum(q * error, (q - 1) * error))

for target_coin in tickers:
    # ── build feature list for Forward-CoVaR Evaluation ───────────────────────
    coin_features = [
        f'{target_coin}_Lev',
        f'{target_coin}_NetAct',
        f'{target_coin}_Val'
    ]

    # Conditionally append DeFi Liquidity Risk (for ETH, SOL, BNB)
    if f'{target_coin}_Liq' in full_df.columns:
        coin_features.append(f'{target_coin}_Liq')

    # Conditionally calculate and append Network Security (for BTC, ETH)
    if target_coin == 'eth' and 'eth_fac_security' in full_df.columns:
        coin_features.append('eth_fac_security')

    elif target_coin == 'btc' and 'btc_HashRate' in full_df.columns:
        # Calculate BTC's log-growth of Hashrate on the fly if it doesn't exist yet
        if 'btc_fac_security' not in full_df.columns:
            full_df['btc_fac_security'] = np.log(full_df['btc_HashRate']).diff()

        # Safely append it to the features list
        coin_features.append('btc_fac_security')

    # Verify which features are actually available
    available_features = [f for f in coin_features if f in full_df.columns]

    if available_features and target_coin in df_dcovar.columns:
        # Align Delta CoVaR (t) with Characteristics & Macro (t-1)
        eval_df = full_df[available_features + macro_vars].shift(1)
        eval_df = eval_df.join(df_dcovar[[target_coin]]).dropna()

        if len(eval_df) < 50:
            print(f"Skipping {target_coin.upper()}: Not enough aligned data.")
            continue
        ...
        y_train = train[target_coin]
        y_test = test[target_coin]

        # Baseline Model (Macro Only)
        X_train_base = sm.add_constant(train[macro_vars])
        X_test_base = sm.add_constant(test[macro_vars])

        model_base = QuantReg(y_train, X_train_base).fit(q=0.05, p_tol=1e-4, max_iter=2000)
        preds_base = model_base.predict(X_test_base)
        loss_base = pinball_loss(y_test, preds_base, q=0.05)

        # Forward-CoVaR Model (Macro + On-Chain)
        X_train_full = sm.add_constant(train[macro_vars + available_features])
        X_test_full = sm.add_constant(test[macro_vars + available_features])

        model_full = QuantReg(y_train, X_train_full).fit(q=0.05, p_tol=1e-4, max_iter=2000)
        preds_full = model_full.predict(X_test_full)
        loss_full = pinball_loss(y_test, preds_full, q=0.05)

        print(f"--- {target_coin.upper()} RESULTS ---")
        print(f"Baseline Loss (Macro Only):    {loss_base:.6f}")
        print(f"Forward-CoVaR Loss (Full):     {loss_full:.6f}")

        if loss_full < loss_base:
            improvement = ((loss_base - loss_full) / loss_base) * 100
            print(f"✅ On-chain data improved tail-risk prediction by {improvement:.2f}%!\n")
        else:
            print(f"❌ Baseline model won out-of-sample.\n")


# In[40]:


print("--- SENSITIVITY ANALYSIS: VARYING TAIL-RISK QUANTILES ---")

test_quantiles = [0.01, 0.05, 0.10]

for target_coin in tickers:
    print(f"\n========================================================")
    print(f"Executing Sensitivity Analysis for {target_coin.upper()}")
    print(f"========================================================\n")

    # 1. Re-prepare eval_df for the specific coin (WITH ALL FEATURES)
    coin_features = [
        f'{target_coin}_Lev',
        f'{target_coin}_NetAct',
        f'{target_coin}_Val'
    ...
    # Safety check: ensure we have features and Delta CoVaR data
    if not available_features or target_coin not in df_dcovar.columns:
        print(f"Skipping {target_coin.upper()}: Missing required features or Delta CoVaR data.")
        continue

    # Construct the aligned dataframe
    eval_df_coin = full_df[available_features + macro_vars].shift(1)
    eval_df_coin = eval_df_coin.join(df_dcovar[[target_coin]]).dropna()

    # Safety check: ensure enough data rows exist after dropping NaNs
    if len(eval_df_coin) < 50:
        print(f"Skipping {target_coin.upper()}: Not enough aligned data after dropping NaNs.")
        continue

    # 2. Split and evaluate
    split = int(len(eval_df_coin) * 0.8)
    train, test = eval_df_coin.iloc[:split], eval_df_coin.iloc[split:]
    y_train, y_test = train[target_coin], test[target_coin]

    print(f"{'Quantile':<10} | {'Baseline Loss':<15} | {'Full Model Loss':<15} | {'Improvement'}")
    print("-" * 65)

    for q_level in test_quantiles:
        # Baseline Model (Macro Only)
        X_train_base = sm.add_constant(train[macro_vars])
        X_test_base = sm.add_constant(test[macro_vars])

        # Fit at the correct tail quantile
        model_base = QuantReg(y_train, X_train_base).fit(q=q_level, p_tol=1e-4, max_iter=2000)
        preds_base = model_base.predict(X_test_base)
        loss_base = pinball_loss(y_test, preds_base, q=q_level)

        # Full Model (Macro + Idiosyncratic Features)
        X_train_full = sm.add_constant(train[macro_vars + available_features])
        X_test_full = sm.add_constant(test[macro_vars + available_features])

        # Fit at the correct tail quantile
        model_full = QuantReg(y_train, X_train_full).fit(q=q_level, p_tol=1e-4, max_iter=2000)
        preds_full = model_full.predict(X_test_full)
        loss_full = pinball_loss(y_test, preds_full, q=q_level)

        # Compare and calculate % improvement
        if loss_full < loss_base:
            imp = f"+{((loss_base - loss_full) / loss_base) * 100:.2f}% ✅"
        else:
            imp = "Negative ❌"

        print(f"q = {q_level:<6} | {loss_base:<15.6f} | {loss_full:<15.6f} | {imp}")
...
# ── fixes applied ──────────────────────────────────────────────────────────────
# 1. macro_vars_clean was never defined → use macro_vars (defined earlier)
# 2. indentation was broken on the two inner `if` blocks
# 3. quantile mismatch: fit at q=0.05, not q=0.50, then evaluate pinball at q=0.05
# ──────────────────────────────────────────────────────────────────────────────

EVAL_Q = 0.05   # single source of truth for the tail quantile

def pinball_loss(y_true, y_pred, q=EVAL_Q):
    error = y_true - y_pred
    return float(((q * error.clip(lower=0)) + ((q - 1) * error.clip(upper=0))).mean())

thesis_results = []
print("--- ENHANCED EVALUATION: STATISTICAL RIGOR & VISUALIZATION ---")
...
for target_coin in tickers:
    ...
    # Conditionally append DeFi Liquidity Risk (for ETH, SOL, BNB)
    if f'{target_coin}_Liq' in full_df.columns:
        coin_features_clean.append(f'{target_coin}_Liq')

    # Conditionally calculate and append Network Security (for BTC, ETH)
    if target_coin == 'eth' and 'eth_fac_security' in full_df.columns:
        coin_features_clean.append('eth_fac_security')
        ...
        # Safely append it to the regression features
        coin_features_clean.append('btc_fac_security')

    # FIX 2: this block was outside the for-loop due to wrong indentation
    if f'{target_coin}_Liq' in full_df.columns:
        coin_features_clean.append(f'{target_coin}_Liq')

    available_features = [f for f in coin_features_clean if f in full_df.columns]

    # FIX 2 (cont.): same indentation problem here
    if not available_features or target_coin not in df_dcovar.columns:
        print(f"  Skipping {target_coin.upper()}: missing features or no df_dcovar column.")
        continue

    # ── 1. prepare aligned data ───────────────────────────────────────────────
    # FIX 1: macro_vars_clean → macro_vars
    eval_df = full_df[available_features + macro_vars].shift(1)
    eval_df = eval_df.join(df_dcovar[[target_coin]]).dropna()

    if len(eval_df) < 100:
        print(f"  Skipping {target_coin.upper()}: only {len(eval_df)} aligned rows.")
        continue
    ...
    # ── 2. baseline model (macro only) ───────────────────────────────────────
    # FIX 1: macro_vars_clean → macro_vars
    # FIX 3: fit at EVAL_Q (0.05), not 0.50
    X_train_base = sm.add_constant(train[macro_vars])
    X_test_base  = sm.add_constant(test[macro_vars])

    res_base     = QuantReg(y_train, X_train_base).fit(q=EVAL_Q, p_tol=1e-4, max_iter=2000)
    preds_base   = res_base.predict(X_test_base)
    loss_base    = pinball_loss(y_test, preds_base, q=EVAL_Q)

    # ── 3. forward-covar model (macro + on-chain) ─────────────────────────────
    # FIX 1 & 3: same corrections as above
    X_train_full = sm.add_constant(train[macro_vars + available_features])
    X_test_full  = sm.add_constant(test[macro_vars + available_features])

    res_full     = QuantReg(y_train, X_train_full).fit(q=EVAL_Q, p_tol=1e-4, max_iter=2000)
    preds_full   = res_full.predict(X_test_full)
    loss_full    = pinball_loss(y_test, preds_full, q=EVAL_Q)

    # ── 4. identify primary on-chain driver ───────────────────────────────────
    char_coeffs = res_full.params[available_features]
    top_driver  = char_coeffs.abs().idxmax()
    driver_val  = char_coeffs[top_driver]

    # ── 5. store results ──────────────────────────────────────────────────────
    improvement = ((loss_base - loss_full) / loss_base) * 100
    thesis_results.append({
        'Coin':           target_coin.upper(),
        'Baseline Loss':  round(loss_base,   6),
        'Full Model Loss':round(loss_full,   6),
        'Improvement %':  round(improvement, 2),
        'Primary Driver': top_driver.split('_')[-1],
        'Driver Beta':    round(driver_val,  4),
    })

    # ── 6. visualisation ──────────────────────────────────────────────────────
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.plot(test.index, y_test,     label='Actual ΔCoVaR',             color='black', alpha=0.3, lw=1)
    ax.plot(test.index, preds_base, label='Macro Baseline',            color='blue',  linestyle='--')
    ax.plot(test.index, preds_full, label='Forward-CoVaR (On-Chain)',  color='red')
    ax.set_title(f"{target_coin.upper()}: Out-of-Sample Systemic Risk Prediction (q={EVAL_Q})")
...
# ── final thesis table ────────────────────────────────────────────────────────
results_table = pd.DataFrame(thesis_results)
print("\n" + "=" * 80)
print("FINAL EMPIRICAL SUMMARY TABLE")
print("=" * 80)
